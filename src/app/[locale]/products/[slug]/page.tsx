...
export default function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // We need to handle the promise params
  const [slug, setSlug] = useState<string>("");
  
  // Use effect to get params
  params.then(p => setSlug(p.slug));

  return <ProductDetailContent slug={slug} locale={locale} isRtl={isRtl} />;
}

function ProductDetailContent({ slug, locale, isRtl }: { slug: string; locale: "ar" | "en"; isRtl: boolean }) {
      ...
      <main className="flex-1 pt-24">
        ...
        <div className="bg-gray-50/80 border-b border-navy-200/10">
          <div className="mx-auto max-w-7xl px-6 py-4">
            <nav className="flex items-center gap-2 text-xs text-navy-400">
              ...
              <span>/</span>
              {cat && <Link href={`/${L}/products`} className="hover:text-cyan-500 transition-colors">{locale === "ar" ? cat.nameAr : cat.nameEn}</Link>}
              <span>/</span>
          ...
          <div className="mx-auto max-w-7xl px-6">
            <div className="grid lg:grid-cols-2 gap-12">
              ...
              <div className={isRtl ? "text-right" : "text-left"}>
                <div className="inline-flex items-center gap-2 rounded-md bg-cyan-50 px-3 py-1 text-xs font-medium text-cyan-600 mb-4">
                  {product.brand}
                </div>
        ...
        {related.length > 0 && (
          <section className="py-20 bg-gray-50/80">
            <div className="mx-auto max-w-7xl px-6">
              <h2 className="text-2xl font-bold text-navy-900 mb-10">
