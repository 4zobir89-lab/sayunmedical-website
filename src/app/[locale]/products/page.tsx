          ...
          <div className="mx-auto max-w-7xl px-6">
            {filtered.length === 0 ? (
              <div className="text-center py-20">
                <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#64748B" strokeWidth="1.5" className="mx-auto mb-4"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                <p className="text-navy-400 mb-2">{locale === "ar" ? "لا توجد منتجات مطابقة" : "No matching products"}</p>
                <p className="text-xs text-navy-300">{locale === "ar" ? "جرب تغيير معايير البحث" : "Try changing your search criteria"}</p>
              </div>
            ) : (
                ...
                {filtered.map((p) => {
                  ...
                  return (
                    <Link key={p.slug} href={`/${L}/products/${p.slug}`}
                      className="group rounded-lg bg-white border border-navy-200/20 hover:border-cyan-500/30 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 overflow-hidden card-shine">
            ...
            {filtered.length > 0 && (
              <p className="text-center text-xs text-navy-300 mt-8">
                {locale === "ar" ? `عرض ${filtered.length} منتج` : `Showing ${filtered.length} products`}
              </p>
            )}
          </div>
        </section>

        <section ref={ctaRef} className="py-24 bg-navy-900 relative overflow-hidden">
          ...
          <div className="mx-auto max-w-2xl px-6 text-center relative z-10">
            ...
            <Link href={`/${L}/contact`}
              className="inline-flex items-center gap-2 rounded-lg bg-cyan-500 px-7 py-3 text-sm font-semibold text-white hover:bg-cyan-400 btn-press transition-all duration-200 shadow-md">
